...
jQuery(document).ready(function() {
	...
	SetEventDaysList(jQuery('#start_date').val(), jQuery('#end_date').val());
...
function SetEventDaysList(start_date, end_date )
{
	...
	jQuery.ajax({
		...
		dataType: "json",
		data : "start_date="+start_date+"&end_date="+end_date+"&act=add",
		success : 
		function(response)
		{
...
<form action="<?php echo site_url('admin/'.$this->config->item('dch_event_module_name').'/save_event/'); ?>" method="post" name="frm_create_event" id="frm_create_event" >
    ...
    <div style="padding:20px;">
        ...
        <div>
			...
			<input type="text" id="start_date" name="start_date" value="<?php echo date('m/d/Y g:i a');?>" class="required" />
       </div><br />
       <div>         
			<label for="end_date">End Date</label>&nbsp;
			<input type="text" id="end_date" name="end_date" value="<?php echo date('m/d/Y g:i a');?>" class="required"  />        
        </div> 
